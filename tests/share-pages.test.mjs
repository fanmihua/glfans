...
test('built pages share one logo with module titles, descriptions and exact destinations', async () => {
  ...
  const routes=['','archive','archive/calendar','radio','tide-words','cp','column','memes','about','about/rights',
...
test('full website retains independently hidden content and private admin metadata', async () => {
  for (const route of ['admin','column/my-secret-words']) {
  ...
  assert.match(root.description,/坑底文学/);
  assert.match(root.description,/电台/);
