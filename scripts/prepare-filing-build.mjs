...
const hiddenAssets = ['column', 'fan-memes', 'meme-game', 'pit-radio'];
...
const hiddenChunks = files.filter(file => /^(?:HomePage|AdminPage|Column|MemesPage|PitRadioPage|WordsTideLab|RepoFilmStrip|en-article|th-article)-.*\.(?:js|css)$/.test(file));
...
await writeFile(new URL('filing-build.json', root), JSON.stringify({
  mode: 'filing',
  publicSections: ['archive', 'cp', 'about'],
